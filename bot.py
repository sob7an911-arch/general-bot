import telebot
import gspread
import json

# --- إعدادات الربط ---
CREDENTIALS_DICT = {
  "type": "service_account",
  "project_id": "rare-mechanic-466808-s2",
  "private_key_id": "52b5890bec4b761f97bec22f635c7bcbcf579713",
  "private_key": "-----BEGIN PRIVATE KEY-----\\nSF0CSDFAwwRrl9bY2SDGjBHk\n-----END PRIVATE KEY-----\n",
  "client_email": "general-bot-service@rare-mechanic-466808-s2.iam.gserviceaccount.com",
}

# --- تهيئة الاتصال ---
gc = gspread.service_account_from_dict(CREDENTIALS_DICT)
sh = gc.open("General_Bot_Data")

BOT_TOKEN = "8606943008:AAGcFvCT73iHY71OOhkw2USy8bNMki72g8s"
bot = telebot.TeleBot(BOT_TOKEN)

# --- أوامر البوت ---
@bot.message_handler(func=lambda m: m.text and m.text.startswith("أضف"))
def add_to_list(message):
    try:
        parts = message.text.split()
        username = parts[1].replace("@", "")
        list_name = parts[3] # المشرفين أو المخربين أو المسجلين
        
        ws = sh.worksheet(list_name)
        ws.append_row([username])
        bot.reply_to(message, f"✅ تمت إضافة @{username} لقائمة {list_name} بنجاح.")
    except Exception as e:
        bot.reply_to(message, "❌ تأكد من صيغة الأمر: أضف @الاسم إلى قائمة المشرفين")

@bot.message_handler(func=lambda m: m.text and m.text.startswith("عرض قائمة"))
def show_list(message):
    list_name = message.text.replace("عرض قائمة", "").strip()
    try:
        ws = sh.worksheet(list_name)
        users = ws.col_values(1)[1:] # عرض من الصف الثاني
        bot.reply_to(message, f"📋 قائمة {list_name}:\n" + "\n".join([f"- @{u}" for u in users]))
    except:
        bot.reply_to(message, "❌ لا توجد قائمة بهذا الاسم.")

bot.infinity_polling()
